...
def extract_array(input_file_line: str) -> list[int]:
    words = input_file_line.split()
    return [int(word) for word in words[1:]]
...
def main():
    ...
    times = extract_array(input_file_lines[0])
    distances = extract_array(input_file_lines[1])
    races = [
        Race(time, distance) for time, distance in zip(times, distances, strict=True)
    ]
    race_results = [
        RaceResult(race, find_winning(race.time, race.record)) for race in races
    ]
    no_winning = [race_result.no_winning for race_result in race_results]
    result = prod(no_winning)
    print(f"{result=}")
