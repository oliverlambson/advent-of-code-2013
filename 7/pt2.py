...
class Hand:
    ...
    @cached_property
    def counter(self) -> Counter[Card]:
        return Counter(self.value)

    @property
    def joker_count(self) -> int:
        return self.counter.get(Card("J"), 0)
...
def safe_count_lookup(counts: list[tuple[Card, int]], idx: int) -> int:
    try:
        count = counts[idx]
    except IndexError:
        return 0
...
def rank_hand(hand: Hand) -> HandRank:
    counts = list(filter(lambda x: x[0] != Card("J"), hand.grouped))
    joker_count = hand.joker_count
    first_count = safe_count_lookup(counts, 0) + joker_count
    second_count = safe_count_lookup(counts, 1)
